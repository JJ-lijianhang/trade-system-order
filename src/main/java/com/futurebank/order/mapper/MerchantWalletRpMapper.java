 ...
 */
public interface MerchantWalletRpMapper extends BaseMapper<MerchantWalletEntity> {

    @Select("select * from tb_merchant_wallet_rp where merchant_id = #{merchantId}")
    List<MerchantWalletEntity> getMerchantWalletByMerchantId(Long merchantId);

//    @Select("select * from tb_merchant_wallet_rp where merchant_id = #{merchantId} and currency = #{currency}")
    ...
    @Update("update tb_merchant_wallet_rp set balance= balance + #{money}, recharge_balance = recharge_balance + #{money}, version = version + 1 where merchant_id = #{merchantId} and currency = #{currency} and version = #{version}")
    int updateMerchantWalletAddRechargeBalance(TradeBeanReq req);

    @Update("update tb_merchant_wallet_rp set balance = balance + #{money}, acquiring_balance = acquiring_balance +  #{money} , version = version + 1 where merchant_id = #{merchantId} and currency = #{currency} and version = #{version} ")
    int updateMerchantWalletAddAcquiringBalance(TradeBeanReq req);

    @Update("update tb_merchant_wallet_rp set balance = balance + #{total} , acquiring_balance = acquiring_balance +  #{moneySub}, collection_balance = collection_balance + #{money} , version = version + 1 where merchant_id = #{merchantId} and currency = #{currency} and version = #{version} ")
    int updateMerchantWalletAddCollectionBalance(BalanceChangeVo req);

    @Update("update tb_merchant_wallet_rp set balance = balance + #{money} ,  freeze_amount = freeze_amount + #{money} , version = version + 1 where merchant_id = #{merchantId} and currency = #{currency} and version = #{version} ")
