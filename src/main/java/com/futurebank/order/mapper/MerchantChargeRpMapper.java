 ...
 */
public interface MerchantChargeRpMapper extends BaseMapper<MerchantChargeEntity> {
    @Select("select * from tb_merchant_charge_rp where reference_no = #{platformOrderNo} and charge_type = #{chargeType} and wallet_type = #{walletType} and charge_event_type = #{chargeEventType}")
    ...
    //    @Select("select * from tb_merchant_charge_rp where reference_no = #{platformOrderNo} and charge_type = #{chargeType} and wallet_type = #{walletType} ")
    @Select("select * from tb_merchant_charge_rp where reference_no = #{platformOrderNo} and imoney < 0 ")
