...
import com.futurebank.order.vo.DateLimitUtilVo;
        ...
        if(!StringUtils.isBlank(data)){
            switch (data){
                ...
                case Constants.SEARCH_DATE_PRE_YEAR:
                    ...
                    break;
                default:
                    List<String> list = FuturebankUtil.stringToArrayStr(data);
                    if(list.size() == 1){
                        throw new CustomException("选择时间参数格式错误，请在 " +
...
//                    }
    ...
    /**
     * get date time string with iso format
     *
    ...
    /**
     * get date time string in some timezone with iso8601 format
     *
     * @param date date time object
    ...
    /**
     * get now time with yyyyMMddHHmmss
     *
     * @return current time
     */
    public static String getSimpleFormatTime() {
        return DateFormatUtils.format(new Date().getTime(), Constants.DATE_TIME_FORMAT_NUM);
    }
